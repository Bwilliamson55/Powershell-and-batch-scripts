#    Send Scriptblock To clients
# V1
# scan all adcomputers that are online, use a interactive input to filter clients, and push a command 
#
# Variables 
...
$outfilename = "$Today -- Commanded list.txt" 
$header = "AD computers that were scanned"
$footer = "`r`n`n$timestamp `r`nClients Sent Commands`r`nPRIVATE`r`n"
$Pushyesno = "NO"
...
write-host "`nSend ScriptBlock to These Clients?`n"
...
foreach($client in $FilteredClients )
{
    ...
    invoke-command -ComputerName $client -scriptblock {Get-ItemProperty HKLM:\Software\Microsoft\Windows\CurrentVersion\Uninstall\* |  ? {$_.displayname -like "*Prey*"} | Select-Object Displayname}
...
write-host "`r`nClients sent commands:`r`n"
